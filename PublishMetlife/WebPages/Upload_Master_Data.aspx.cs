...
public partial class WebPages_Upload_Master_Data : System.Web.UI.Page
{
    ...
    public WebPages_Upload_Master_Data()
    {
        oPRP = new AssetAcquisition_PRP();
    }
    ~WebPages_Upload_Master_Data()
    {
        oPRP = null;
        oDAL = null;
    }
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!IsPostBack)
            {
                string _strRights = clsGeneral.GetRights("IMPORT_MASTER", (DataTable)Session["UserRights"]);
                clsGeneral._strRights = _strRights.Split('^');
                clsGeneral.LogUserOperationToLogFile(Session["CURRENTUSER"].ToString(), Session["COMP_NAME"].ToString(), "IMPORT_MASTER");
    ...
    {
        new AssetAcquisition_DAL(Session["DATABASE"].ToString()).SaveAuditLogin(Session["CURRENTUSER"].ToString(), Session["COMPANY"].ToString(), "Upload Master Data", "Master Data Upload", "Master Data upload by user id" + Session["CURRENTUSER"].ToString());
        try
        {
            ...
            else if (ddlImportType.SelectedValue.ToString() == "USER")
            {
                Session["USER"] = null;
                UploadUserFile();
            }
    ...
    protected void btnSubmit_Click(object sender, ImageClickEventArgs e)
    {
        try
        {
            ...
            {
                SaveUserDetails();
                ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "ShowAlert", "ShowAlert('Please Note : User data is saved successfully.');", true);
            }
        }
        catch (Exception ex)
        {
     ...
     private void SaveUserDetails()
    {
        if (Session["USER"] != null)
        {
            ...
            try
            {
                ...
                dtFileData = (DataTable)Session["USER"];
                string group = "";
                string userid = "";
                string username = "";
                string empid = "";
                ...
                {
                    if (dtFileData.Columns.Count != 6)
                    {
                        ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "ShowAlert", "ShowAlert('Please Note : invalid file format file should contains 6 columns. " + (iCnt + 1).ToString() + ".');", true);
                        bInValid = true;
                        return;
                    }
                    for (iCnt = 0; iCnt < dtFileData.Rows.Count; iCnt++)
                    {
                        ...
                        if (dtFileData.Rows[iCnt][0].ToString().Trim() != "")
                            location = dtFileData.Rows[iCnt][0].ToString().Replace("'", "`").Trim();
                        else
                        {
                            ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "ShowAlert", "ShowAlert('Please Note : user location  is left blank at row no. " + (iCnt + 1).ToString() + ".');", true);
                            bInValid = true;
                            break;
                        }
                        if (dtFileData.Rows[iCnt][1].ToString().Trim() != "")
                            username = dtFileData.Rows[iCnt][1].ToString().Replace("'", "`").Trim();
                        else
                        {
                            ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "ShowAlert", "ShowAlert('Please Note : User/ employee name  is left blank at row no. " + (iCnt + 1).ToString() + ".');", true);
                            bInValid = true;
                            break;
                        }
                        if (dtFileData.Rows[iCnt][2].ToString().Trim() != "")
                            empid = dtFileData.Rows[iCnt][2].ToString().Replace("'", "`").Trim();

                        if (dtFileData.Rows[iCnt][3].ToString().Trim() != "")
                            userid = dtFileData.Rows[iCnt][3].ToString().Replace("'", "`").Trim();
                        else
                        {
                            ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "ShowAlert", "ShowAlert('Please Note : user id  is left blank at row no. " + (iCnt + 1).ToString() + ".');", true);
                            bInValid = true;
                            break;
                        }
                        //  userEmail = dtFileData.Rows[iCnt][4].ToString().Trim().ToUpper();

                        if (dtFileData.Rows[iCnt][4].ToString().Trim() != "")
                        {
                            if (System.Text.RegularExpressions.Regex.IsMatch(dtFileData.Rows[iCnt][4].ToString().Trim(), @"^([\w\.\-]+)@([\w\-]+)((\.(\w){2,3})+)$") == true)
                                userEmail = dtFileData.Rows[iCnt][4].ToString().Replace("'", "`").Trim();
                            else
                            {
                                ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "ShowAlert", "ShowAlert('Please Note : user E-mail format is invalid at row no. " + (iCnt + 1).ToString() + ".');", true);
                                bInValid = true;
                                break;
                            }
                        }
                        ...
                        createdby = Session["CURRENTUSER"].ToString();
                        if (Convert.ToString(dtFileData.Rows[iCnt][5]) != "" && dtFileData.Rows[iCnt][5].ToString().ToLower() == "active")
                            status = Convert.ToBoolean(1);
                        else if (Convert.ToString(dtFileData.Rows[iCnt][5]) != "")
                            status = Convert.ToBoolean(dtFileData.Rows[iCnt][5]);

                        oDAL.Uploaduser(empid, location, username, userid, userEmail, status, createdby);
                    }
                }
                if (!bInValid)
                    ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "ShowErrMsg", "ShowErrMsg('Please Note : user data is saved successfully.');", true);
            }
            catch (Exception ex)
            {
                ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "ShowAlert", "ShowAlert('Please Note : An error has occured while uploading employee details, check file data.');", true);
        ...
        {
            ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "ShowAlert", "ShowAlert('Please Note : Please Upload user master file first.');", true);
        }
    }
    #endregion
    ...
    private void SaveVendorDetails()
    {
        if (Session["VENDOR"] != null)
        {
            ...
            try
            {
                ...
                if (!bInValid)
                    ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "ShowErrMsg", "ShowErrMsg('Please Note : Vendor data is saved excep duplicate vendor successfully.');", true);
            }
            catch (Exception ex)
            {
        ...
        {
            ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "ShowAlert", "ShowAlert('Please Note : Upload vendor master file first.');", true);
        }
    }
    #endregion
    ...
    {
        string strFileName = FileUploadUser.FileName;
        if (strFileName != "")
        {
            String fileExt = System.IO.Path.GetExtension(FileUploadUser.FileName);
            if (fileExt.ToUpper() != ".XLSX" && fileExt.ToUpper() != ".XLS")
            {
                ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "ShowErrMsg", "ShowErrMsg('Please Note : Select an excel file (.xls/.xlsx) only.');", true);
                return;
    ...
    private bool UploadFile()
    {
        ...
        if (ddlImportType.SelectedValue.ToString() == "USER")
        {
            fileExt = System.IO.Path.GetExtension(FileUploadUser.FileName);
            if (FileUploadUser.HasFile)
            {
                if (fileExt == ".xls" || fileExt == ".XLS" || fileExt == ".xlsx")
                {
                    strFilePath = Request.PhysicalApplicationPath + "UploadedFiles\\" + FileUploadUser.FileName;
                    File.Delete(strFilePath);
                    FileUploadUser.SaveAs(strFilePath);
                    bRes = true;
                }
            }
        }
