...
public partial class AssetTagPersonslization : System.Web.UI.Page
{
    ...
    string strDupFilePath = @"C:\ATS\";
     ...
     public AssetTagPersonslization()
    {
        oPRP = new TagMaster_PRP();
    }
     ~AssetTagPersonslization()
    {
        oPRP = null;
        oDAL = null;
    }
    ...
    private void SaveVendorDetails()
    {
        ...
        try
        {
            ...
            for (iCnt = 0; iCnt < dtFileData.Rows.Count; iCnt++)
            {
                ...
                oPRP.HostName = dtFileData.Rows[iCnt][2].ToString().Replace("'", "`").Trim();
                oPRP.AssetTag = dtFileData.Rows[iCnt][3].ToString().Replace("'", "`").Trim();
                ...
                oDAL.UploadAssetTagDetails(oPRP);
            }
            if (!bInValid)
