...
public partial class Report_AuditHistory : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!IsPostBack)
            {
                string _strRights = clsGeneral.GetRights("AUDIT_HISTORY", (DataTable)Session["UserRights"]);
                clsGeneral._strRights = _strRights.Split('^');
                clsGeneral.LogUserOperationToLogFile(Session["CURRENTUSER"].ToString(), Session["COMP_NAME"].ToString(), "AUDIT_HISTORY");
    ...
    protected void btnExport_Click(object sender, ImageClickEventArgs e)
    {
        try
        {
            ...
            {
                DataTable dt = (DataTable)Session["RptAH"];
                ...
                //hw.WriteLine("<br>");
                //hw.WriteLine("<b><u><font size='4'> Audit Log Report</font></u></b>");
                //hw.WriteLine("<br>");
    ...
    private void GetReports()
    {
        ...
        oPRP.Type = "AUDITREPORT";
        gvRptGatePass.DataSource = null;
        DataTable dt = new DataTable();
        dt = oDAL.GetReportDetails(oPRP);
        if (dt.Rows.Count > 0)
        {
            gvRptGatePass.DataSource = Session["RptAH"] = dt;
