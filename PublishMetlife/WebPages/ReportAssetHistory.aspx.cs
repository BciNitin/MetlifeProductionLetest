...
public partial class Report_AuditHistory : System.Web.UI.Page
{
    bool bExpire = false;
    Report_DAL oDAL;
    Report_PRP oPRP;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!IsPostBack)
            {
                string _strRights = clsGeneral.GetRights("ASSET_HISTORY", (DataTable)Session["UserRights"]);
                clsGeneral._strRights = _strRights.Split('^');
                clsGeneral.LogUserOperationToLogFile(Session["CURRENTUSER"].ToString(), Session["COMP_NAME"].ToString(), "ASSET_HISTORY");
    ...
    protected void btnSubmit_Click(object sender, ImageClickEventArgs e)
    {
        try
        {
            ...
            new AssetAcquisition_DAL(Session["DATABASE"].ToString()).SaveAuditLogin(Session["CURRENTUSER"].ToString(), Session["COMPANY"].ToString(), "Asset Allocation", "Asset History Report", "Asset History Report generate by user id" + Session["CURRENTUSER"].ToString());
            //if (clsGeneral._strRights[0] == "0")
            //{
    ...
    protected void btnExport_Click(object sender, ImageClickEventArgs e)
    {
        try
        {
            ...
            {
                DataTable dt = (DataTable)Session["RptAHR"];
                ...
                //hw.WriteLine("<br>");
                //hw.WriteLine("<b><u><font size='4'>Asset History Report</font></u></b>");
                //hw.WriteLine("<br>");
                //dgGrid.HeaderStyle.Font.Bold = true;
                //dgGrid.DataBind();
                //dgGrid.RenderControl(hw);
                //Response.ContentType = "application/vnd.ms-excel";
                //Response.AddHeader("Content-Disposition", "attachment;filename=HistoryReport.xls");
    ...
    private void GetReports()
    {
        ...
        if (txtActiveInAssetCode.Text.Trim() != "")
        {
            ...
            {
                oPRP.TagID = txtActiveInAssetCode.Text.Split('|')[0].Trim();

            }
            else
            {
                oPRP.TagID = txtActiveInAssetCode.Text.Trim();
            }

        }

        oPRP.CompCode = Session["COMPANY"].ToString();
        oPRP.Type = "ASSETHISTORY";
        gvRptGatePass.DataSource = null;
        DataTable dt = new DataTable();
        dt = oDAL.GetReportDetails(oPRP);
        if (dt.Rows.Count > 0)
        {
            gvRptGatePass.DataSource = Session["RptAHR"] = dt;
